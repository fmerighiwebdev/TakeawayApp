...
export async function getProductCustomizations(productId, tenantId) {
  ...
  const { data: doughs, error: doughsError } = await supabaseServer
    .from("dough_options")
    .select("id, name, price")
    .eq("product_id", productId);
  ...
  const { data: cookings, error: cookingsError } = await supabaseServer
  ...
  const { data: spiceLevels, error: spiceError } = await supabaseServer
    .from("spice_levels")
    .select("id, label")
    .eq("product_id", productId);
  ...
  const { data: removals, error: removalsError } = await supabaseServer
