...
import { normalizeEmail, normalizePhone } from "@/lib/orders/orderRequest";
import {
  getPaginationRange,
  getTotalPages,
  normalizePage,
...
} from "@/lib/shared/listing";

function applyCustomerSearch(query, searchTerm) {
  ...
  if (normalizedPhoneSearch) {
    filters.push(`normalized_phone.ilike.%${normalizedPhoneSearch}%`);
  }
...
export async function getCustomers(tenantId, options = {}) {
  if (!tenantId) {
    throw new Error("tenantId mancante.");
  }
  ...
  let query = supabaseServer
    .from("customers")
    .select(
      "id, full_name, normalized_email, normalized_phone, orders_count, total_spent, last_order_at, created_at",
      { count: "exact" },
    )
    .eq("tenant_id", tenantId);
