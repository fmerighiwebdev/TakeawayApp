...
export async function getTenantOrdersByStatus(tenantId, status) {
  if (!tenantId) {
    throw new Error("Tenant ID obbligatorio");
  }
  if (!status) {
    throw new Error("Stato obbligatorio");
  }
...
export async function updateOrderStatus(tenantId, orderId, newStatus) {
  if (!tenantId) {
    throw new Error("Tenant ID obbligatorio");
  }
  if (!orderId) {
