    ...
    .insert([
      {
        ...
        percent_off: orderData.percent_off || null,
        discounted_price: orderData.discounted_price || null,
...
export async function getPastOrdersByTenantId(tenantId) {
  if (!tenantId) {
    throw new Error("Tenant ID obbligatorio");
  }
  ...
  const startOfMonth = new Date(
    now.getFullYear(),
    now.getMonth(), 1, 0, 0, 0, 0
  );
  const startOfNextMonth = new Date(
    now.getFullYear(),
    now.getMonth() + 1, 1, 0, 0, 0, 0
  );
...
export async function getPastOrdersByTenantIdWithDetails(tenantId) {
