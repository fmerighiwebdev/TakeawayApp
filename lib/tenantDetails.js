...
export async function getTenantCompletion(tenantId) {
  ...
  if (error || !tenantData) {
    throw new Error(
      error?.message ||
        "Impossibile recuperare lo stato di completamento del tenant",
    );
  }
...
export async function getTenantSMTPConfig(tenantId) {
  ...
  if (error || !tenantData) {
    throw new Error(
      error?.message || "Configurazione SMTP del tenant non trovata",
    );
  }
...
export async function getTenantUI(tenantId) {
  ...
  if (error || !tenantData) {
    throw new Error(error?.message || "Configurazione UI del tenant non trovata");
  }
  
  return tenantData.ui;
}
