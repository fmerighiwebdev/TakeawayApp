...
function formatSingleChoice(label, value) {
  if (!value) return "";
...
function formatOrderItems(items) {
  ...
  return items
    .map((item) => {
      ...
      const extras = normalizeOptionValues(item.extras);
      const removals = normalizeOptionValues(item.removals);

      const dough = formatSingleChoice("Impasto", item.dough);
      const cooking = formatSingleChoice("Cottura", item.cooking);
      const spice = formatSingleChoice("Piccante", item.spice);
      ...
      const details = [
        ...
        formatOptionGroup("Rimozioni", removals),
      ].filter(Boolean);
