...
import { getTenantContext } from "../tenant/tenantDetails";
import { getOrderByIdWithDetails } from "../orders/orders";
...
export async function sendOrderPostponementEmail({
  ...
  const [tenantContext, tenantOrder] = await Promise.all([
    getTenantContext(tenantId),
    getOrderByIdWithDetails(tenantId, orderId),
  ]);
  ...
  const tenantSMTPConfig = tenantContext.smtp;
