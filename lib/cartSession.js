...
const LEGACY_CART_STORAGE_KEY_PREFIX = "cart-storage-";
...
function getLegacyCartStorageKey() {
  ...
  return `${LEGACY_CART_STORAGE_KEY_PREFIX}${hostname}`;
}
...
export function removeLegacyCartFromLocalStorage() {
  ...
  const legacyCartStorageKey = getLegacyCartStorageKey();
  const keysToRemove = [];
  ...
  for (let index = 0; index < window.localStorage.length; index += 1) {
    ...
    if (
      key === legacyCartStorageKey ||
      key.startsWith(LEGACY_CART_STORAGE_KEY_PREFIX)
    ) {
